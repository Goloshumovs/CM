...
def Max_Min(razb, tx, bx, ty, by, tz, bz, function):
    ...
    if kp3.order()[0]=='y' and kp3.order()[1]=='x':
        z_max = Max_Min2(Y, X, tz)[0]
        z_min = Max_Min2(Y, X, bz)[1]
    else:
        z_max= Max_Min2(X, Y, tz)[0]
        z_min= Max_Min2(X, Y, bz)[1]
...
def Max_Min3(X, Y, function):
    ...
    return mmax, mmin
def Max_Min4(X, Y, Z, function):
    mmax = function(X[0], Y[0], Z[0])
    mmin = function(X[0], Y[0], Z[0])
    for i in range(len(X)):
        for j in range(len(Y)):
            for k in range(len(Z)):
                if function(X[i], Y[j], Z[k])>mmax:
                    mmax=function(X[i], Y[j], Z[k])
                if function(X[i], Y[j], Z[k])<mmin:
                    mmin=function(X[i], Y[j], Z[k])
    return mmax, mmin
...
def Monte_Karlo3_4D(N, razb):
    ...
    X = np.linspace(x_min, x_max, razb)
    Y = np.linspace(y_min, y_max, razb)
    Z = np.linspace(z_min, z_max, razb)
    q_max, q_min = Max_Min4(X, Y, Z, kp3.function)
    cntP = 0
    cntO = 0
    Pnts = np.ones((5, N))
    ...
    for i in range(N):
        ...
        yi=y_min+(y_max-y_min)*random.random()
        zi=z_min+(z_max-z_min)*random.random()
        qi=q_min+(q_max-q_min)*random.random()
        Pnts[0][i]=xi
        Pnts[1][i]=yi
        Pnts[2][i]=zi
        Pnts[3][i]=qi
        if kp3.tz(xi, yi)>zi and kp3.bz(xi, yi)<zi:
            if kp3.ty(xi, zi)>yi and kp3.by(xi, zi)<yi:
                if kp3.tx(yi, zi)>xi and kp3.bx(yi, zi)<xi:
                    if kp3.function(xi, yi, zi)>0 and kp3.function(xi, yi, zi)>qi and qi>0:
                        cntP+=1
                        Pnts[4][i] = 0
                    elif kp3.function(xi, yi, zi)<0 and kp3.function(xi, yi, zi)<qi and qi<0:
                        cntO += 1
                        Pnts[4][i] = 0
    dol1=cntP/N
    dol2=cntO/N
    S=(x_max-x_min)*(y_max-y_min)*(z_max-z_min)*(q_max-q_min)
    print(S)
    res=S*dol1-S*dol2
    return res, Pnts
